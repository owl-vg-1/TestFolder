...
if ($_SESSION['ban']>=time()){
	echo "Вы забанены за использование нецензурных выражений!<br>";
	$time_ban = $_SESSION['ban'] - time();
	$minutes = floor($time_ban % 3600 / 60);
	$seconds = $time_ban % 60;
	echo "И не можете писать в гостевой книге $minutes минуты $seconds секунд.";
}
//Проверка ввод ника
elseif (empty($nik)) {
	echo "<h2>Вы не ввели имя!</h2>";

//Проверка ввод сообщения
}elseif (empty($textN)) {
	echo "<h2>Вы не ввели сообщение!</h2>";

//Проверка на использование нецензурных слов
} elseif  (count(bad_word($textN))>0) {
	echo "<h2>Вы забанены за использование нецензурных выражений!</h2>";
	$_SESSION['ban']=time()+80;
	$arr = bad_word($textN);
